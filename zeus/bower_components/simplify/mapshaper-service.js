...
/*   new SimpleButton('#simplify-settings-btn').on('click', function () {
  ...
  new SimpleButton('#closeNavBtn').on('click', function () {
    slider.reset();
  });
...
/*   new ModeButton('#simplify-btn', 'simplify');
 ...
 /*  function initMenu() {
  ...
  function getSimplifyOptions() {
    var optsMethod = ['dp', 'visvalingam', 'weighted_visvalingam']
    var method = optsMethod[2] || null;
    return {
      method: method,
      percentage: _value,
      no_repair: true,
...
/*   function submitFiles(quickView) {
  ...
  self.readFileContent = function (name, content) {
...
var ExportControl = function (model, simplify) {
  ...
  if (!gui.exportIsSupported()) {
    ...
    new SimpleButton('#exportt-btn').on('click', onExportClick);
  ...
  function exportMenuSelection(done) {
    ...
    try {
      ...
      var gjSimplify = api.internal.decodeString(files[0].content, 'utf8');
      gjSimplify = JSON.parse(gjSimplify)
      //La funcion recibe el geojson simplificado para insertarlo en el mapa 
      //Esta funcion se encuentra en quadtree.js
      displaySimplifyGJ(gjSimplify)
      simplify.resetSlider(); 
...
gui.startEditing = function () {
  var model = new Model(),
    dataLoaded = false,
    map, repair, simplify, importControl;
  gui.startEditing = function () { };
  ...
  importControl = new ImportControl(model, gui.getImportOpts());
  new ExportControl(model, simplify);
  //new LayerControl(model, map);
  simplifyGeojson = new SimplifyGeojson(importControl, simplify, model);
...
function SimplifyGeojson(importControl, simplify, model) {

  var self = this;



  self.loadGeojson = function (geojson) {
   ...
   /*  try{
    ...
    importControl.readFileContent('name', geojson);
    El('body').removeClass('queued-files');
    El('body').removeClass('splash-screen');
